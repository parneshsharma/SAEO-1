...
def run_data_point(yy,x, encoder, model, measurement, backend, n_shots):
...
def run(X, encoder, model,yy,
    ...
    if len(X.shape) == 1:
        # Single data example
        dataset_size = 1
        predictions = np.array(run_data_point(yy,X, encoder, model, measurement, backend, n_shots))
        ...
        for i, x in enumerate(X):
            predictions[i] = run_data_point(yy,x, encoder, model, measurement, backend, n_shots)
    predictions =predict_class_(predictions, yy)
    return predictions
